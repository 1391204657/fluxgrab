...
window.FluxGrabAnalytics = (function () {
  ...
  document.addEventListener("click", function (e) {
    var el = e.target.closest("[data-download-win], [data-buy-link], .ad-slot, .vpn-ad a");
    if (!el) return;
    if (el.matches("[data-download-win]")) send("download_win");
    else if (el.matches("[data-buy-link]")) send("buy_click");
    else if (el.closest(".vpn-ad") || el.closest(".ad-slot")) send("ad_click");
  });

  document.querySelectorAll(".ad-slot, #vpnAd").forEach(function (node) {
